...
implementation {
  ...
  event void Boot.booted() {
    ...
    //hjy change:maxSend
    maxSend = 300;
    //hjy change:sense_t channelState
    channelState.id = TOS_NODE_ID;
...
//	rssiData = data;
	//channelState.regRssi = data;
	//call SerialSend.send(0x1818, &channelState, sizeof(channelState));
   ...
   */
   ...
   {
      //call SerialSend.send(0x1817, &channelState, sizeof(channelState));
      call SerialSend.send(0x1817, &local, sizeof(local));
      //hjy change:maxSend
    /*
       if (maxSend != 0) {
    if (reading == NREADINGS) {
